...
4. 作業手順
    4.1 ビルド環境設定
        ...
        4.1.4 依存関係追加
    4.2 マニフェストファイルの記述
        4.2.1 センサー情報取得に関する利用者権限追加
        4.2.2 端末位置情報の追跡取得に関する利用者権限追加
...
`SINETStreamHelper`ライブラリが収集したセンサー情報をJSON形式で出力する際、
付加情報としてAndroid端末の位置情報(緯度、経度)を含められるようになっている。
Android端末を固定的な場所に設置する場合は静的に位置情報を設定するとして、
これを移動しながら使う場合は位置情報を追跡取得できるような機能も用意する。


## 2. モジュール構成

ユーザアプリケーションが`SINETStreamHelper`ライブラリを利用する場合のモジュール構成を以下に示す。
...
本ライブラリーを利用するアプリケーションは、端末の位置情報にアクセスするため以下の実行時権限をマニフェストファイルに追加で記述する必要がある。
...
Android8.0以降の場合、
アプリケーションの初回起動時に権限検査が実施されるだけでなく、
その利用権限は利用者の手動操作によりいつでも`On/Off`可能となる。
