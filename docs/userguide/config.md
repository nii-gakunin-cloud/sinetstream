...
ヘッダ部分が存在しない場合は従来フォーマット(version 1; コンフィグ部のみ)だとみなす。
...
config:
  ...
  ...
  type_spec:
    {メッセージングシステム固有のパラメータ1}: {設定値1}
    {メッセージングシステム固有のパラメータ2}: {設定値2}
    ...
```
...
ヘッダパラメータには以下のものを指定する。

* version
    * 設定ファイルのフォーマットバージョン
    * 指定できる値は `2` または `3` である。
    * 省略した場合は最新のフォーマット(現在は3)が指定されたものとみなす。
* fingerprint
...
type_spec:のメッセージングシステム固有のパラメータ
(config version 1 or 2ではその他のパラメータ)
として指定できる値はメッセージングシステムによって異なる。
例えば以下のような値が指定できる。
...
|---|---|---|---|
|ca_certs|str(Path)|ssl_cafile|tls:ca_certs|
|certfile|str(Path)|ssl_certfile|tls:certfile|
|keyfile|str(Path)|ssl_keyfile|tls:keyfile|
|ciphers|str|ssl_ciphers |tls:ciphers|
