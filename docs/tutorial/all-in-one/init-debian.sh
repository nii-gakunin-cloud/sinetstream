...
setup_mosquitto_ssl() {
  ...
  mkdir -p /var/lib/mosquitto /run/mosquitto
  chown mosquitto:mosquitto /var/lib/mosquitto /run/mosquitto
...
setup_http_server() {
  expose_cert
  if ! grep -q program:http /etc/supervisor/supervisord.conf; then
