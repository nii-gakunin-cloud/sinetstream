...
このディレクトリのテストでは実際のS3サーバーを用いてテストを行います。
そのため事前にS3サーバーを準備する必要があります。

テストに利用するS3サーバーは以下の２つの方法を選択できます。
...
テスト結果の詳細については
`sinetstream-java/plugin-mqtt/build/reports/tests/test/index.html` 
にレポートが作成されています。
