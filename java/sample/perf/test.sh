...
PKGDIR="repo/sinetstream-java/sample/perf/sinetstream-binary-producer-1.1.0"

SERVICE_KAFKA=service-kafka-aws
SERVICE_MQTT=service-mqtt-aws
...
RUN_kafka() {
        RUN "$TOPO-kafkaS-$MSZ" --service "$SERVICE_KAFKA" sinetstream-binary-producer sinetstream-binary-consumer
        RUN "$TOPO-kafkaN-$MSZ" --broker "$BROKER_KAFKA" kafka-binary-producer kafka-binary-consumer
}
RUN_mqtt() {
        RUN "$TOPO-mqttS-$MSZ" --service "$SERVICE_MQTT" sinetstream-binary-producer sinetstream-binary-consumer
        RUN "$TOPO-mqttN-$MSZ" --broker "$BROKER_MQTT" mqtt-binary-producer mqtt-binary-consumer
}
