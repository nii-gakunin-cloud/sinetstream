...
RUN_kafka() {
        RUN "$TOPO-kafkaS-$MSZ-crypto" --service "$SERVICE_KAFKA" sinetstream-binary-producer sinetstream-binary-consumer
        #RUN "$TOPO-kafkaN-$MSZ-crypto" --broker "$BROKER_KAFKA" kafka-binary-producer kafka-binary-consumer
}
RUN_mqtt() {
        RUN "$TOPO-mqttS-$MSZ-crypto" --service "$SERVICE_MQTT" sinetstream-binary-producer sinetstream-binary-consumer
        #RUN "$TOPO-mqttN-$MSZ-crypto" --broker "$BROKER_MQTT" mqtt-binary-producer mqtt-binary-consumer
}
