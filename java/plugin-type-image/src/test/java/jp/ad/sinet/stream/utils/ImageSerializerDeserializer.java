...
import jp.ad.sinet.stream.api.valuetype.ImageDeserializer;
...
class ImageSerializerDeserializer {
    ...
    @Test
    void badDataDeserialize() {
        ImageSerializer<RenderedImage> ser = new ImageSerializer<>();
        byte[] bytes = ser.serialize(this.image);
        bytes[0] = '\0';
        ImageDeserializer des = new ImageDeserializer();
        assertThrows(SinetStreamIOException.class,  () -> des.deserialize(bytes));
    }

    @Test
    void badDataSerialize() {
        ImageSerializer<RenderedImage> ser = new ImageSerializer<>("xxx");
        assertThrows(SinetStreamIOException.class,  () -> ser.serialize(image));
    }
}
