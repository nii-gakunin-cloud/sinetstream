...
generate_client_cert() {
  ...
  if [[ ! -f ${cert_dir}/client0.key ]]; then
    ...
    openssl rsa -aes256 -in ${cert_dir}/client0.key \
      -out ${cert_dir}/client0-enc.key -passout pass:client1-pass
    openssl pkcs12 -export -in ${cert_dir}/client0.crt \
      -inkey ${cert_dir}/client0.key -name client0 -caname private-ca \
      -CAfile ${CATOP}/cacert.pem -passout pass:client0-pass \
      -out ${cert_dir}/client0.p12
  fi
}
...
generate_client_certs() {
  generate_client_cert
  generate_bad_client_cert
  cp ${KAFKA_CERTS_DIR}/ca.p12 /srv/http/
}
